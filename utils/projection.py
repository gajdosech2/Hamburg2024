...
def eye_views(coco_json, head_image_path, image_id, coords):
    ...
    for c in coords:
        ...
        left_pixel, left_vis_img = left_image_processor.coordinate_to_pixel(img=left_image, coordinate=coordinate, camera_tfrm=left_camera_tfrm, verbose=False)
        cv2.circle(left_vis_img, (int(left_pixel[0]), int(left_pixel[1])), radius=5, color=(0, 255, 0), thickness=4) 
        cv2.imwrite('/home/g/gajdosech2/Hamburg2024/work_dirs/debug/left.png', left_vis_img)

        right_pixel, right_vis_img = right_image_processor.coordinate_to_pixel(img=right_image, coordinate=coordinate, camera_tfrm=right_camera_tfrm, verbose=False)
        cv2.circle(right_vis_img, (int(right_pixel[0]), int(right_pixel[1])), radius=5, color=(0, 255, 0), thickness=4) 
        cv2.imwrite('/home/g/gajdosech2/Hamburg2024/work_dirs/debug/right.png', right_vis_img)
    ...
    left_masks = segmentation_masks("/home/g/gajdosech2/Hamburg2024/", left_image, left_coords, left_coords)
    right_masks = segmentation_masks("/home/g/gajdosech2/Hamburg2024/", right_image, right_coords, left_coords)
